...
>>>>>>> Stashed changes
    func loadData() {
        ...
        if let latestWeatherData = sortedWeatherData.last {
            ...
            if timeDifference < oneHourInSeconds && cityNameMatches {
                ...
                print("Stored data is outdated or city name has changed. Finding closest data point.")
                if let closestWeatherData = findClosestWeatherData(to: currentTime, in: sortedWeatherData) {
                    print("Using closest stored data: \(closestWeatherData)")
                    loadWeatherDetails(for: closestWeatherData)
                } else {
                    print("No valid stored data found. Fetching from API.")
                    fetchWeatherFromAPI()
                }
            }
        } else {
    ...
    private func findClosestWeatherData(to currentTime: TimeInterval, in sortedWeatherData: [CDWeatherInfo]) -> CDWeatherInfo? {
        var closestWeatherData: CDWeatherInfo?
        var smallestTimeDifference = Double.greatestFiniteMagnitude
        
        for weatherData in sortedWeatherData {
            let timeDifference = abs(currentTime - TimeInterval(weatherData.dt))
